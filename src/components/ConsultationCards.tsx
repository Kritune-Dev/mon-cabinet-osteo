...
const cards = [
  {
    ...
    label: 'Adultes',
    description: 'Gestion des douleurs, troubles musculo-squelettiques, stress, troubles digestifs…',
    href: '/adulte',
  ...
  },
  {
    ...
    label: 'Sportifs',
    description: 'Préparation, récupération, prévention des blessures, suivi personnalisé.',
    href: '/sportif',
  ...
  },
  {
    ...
    description: "Accompagnement de la grossesse, soulagement des douleurs lombaires, préparation à l'accouchement.",
    href: '/femme-enceinte',
  ...
  },
  {
    ...
    description: 'Suivi du développement, troubles du sommeil, coliques, plagiocéphalie…',
    href: '/enfants',
    ...
    <ul className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-2" aria-label="Types de patients">
      {cards.map(card => (
        <li
          key={card.id}
          id={card.id}
          className="group bg-[#F5E9E3] rounded-lg p-4 shadow flex flex-col items-start cursor-pointer transition-transform duration-150 hover:scale-[1.025] active:scale-[0.98] focus-within:ring-2 focus-within:ring-[#E08A3D]"
          tabIndex={0}
          role="button"
          aria-label={card.aria}
          onClick={() => window.location.href = card.href}
          onKeyDown={e => { if (e.key === 'Enter' || e.key === ' ') window.location.href = card.href }}
        >
          <span className="font-bold text-lg mb-1" style={{ color: "#1a120b" }}>{card.label}</span>
